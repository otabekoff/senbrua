...
export class Recorder extends GObject.Object {
    constructor() {
        super();
        this._duration = 0;
        this._current_peak = 0;
        this.peaks = [];
        this.noiseSuppressionPreference = Settings.get_boolean("noise-reduction-enabled");
    ...
    configurePipeline(enableNoise) {
        ...
        if (enableNoise) {
            ...
            let rnnoiseElement = this.tryMakeElement("audiornnoise", "audiornnoise");
            ...
            if (rnnoiseElement) {
                const resample = this.makeElement("audioresample", "rnnoiseResample");
                this.linkElements(tail, resample);
                tail = resample;
                const capsFilter = this.makeElement("capsfilter", "rnnoiseCaps");
                // audiornnoise uses F32LE, rnnoise uses S16LE
                const rnnoiseCaps = Gst.Caps.from_string(useF32Format
                ...
                tail = rnnoiseElement;
                const postConvert = this.makeElement("audioconvert", "postConvert");
                this.linkElements(tail, postConvert);
                tail = postConvert;
            }
            else {
                log("RNNoise plugin not available - continuing without noise reduction.");
            }
        }
    ...
    linkElements(src, dest, caps) {
        const success = caps ? src.link_filtered(dest, caps) : src.link(dest);
        if (!success) {
            throw new Error(`Failed to link ${src.get_name()} to ${dest.get_name()}`);
        }
    }
    getChannel() {
    ...
    set current_peak(peak) {
        if (this.peaks) {
            ...
            if (Number.isFinite(normalizedPeak) &&
